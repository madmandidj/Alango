...
------------------
	...
	b. Anatomy of the game engine. 	
		...
		From BSEngine.h:
			...
			- BS_EngineError_t BS_Engine_init(int argc, char** argv)
			  This funtion parses the CLI arguments passed as they come from the 'main' function and initializes all the game mechanics accordingly.
			  			  
			  CLI arguments are:
				- "-p1" <DLL_name>. Specifies the name of the DLL file for player 1 Battlebot.
				- "-p2" <DLL_name>. Specifies the name of the DLL file for player 1 Battlebot.
				- "-rounds" <number of rounds>. Specifies the number of rounds to play. Positive integer value.
				- "-display" <display rate>. Specifies the display refresh delays in ms. Setting to 0 will disable the display.
				- "-log" <text log mode>. Specifies the text log mode. Currently only 0 (off) and 1 (on) are supported.
			  
			  If a parameter is not specified, it is replaced by its default value.
			  
			  The default values are:
				- Player 1 DLL: "BSPlayer1.dll"
				- Player 2 DLL: "BSPlayer2.dll"
				- Number of rounds: 10
				- Display rate: 100
				- Text log mode: 1
			  
			- void BS_Engine_play(void)
...
2018-08-05	- Implemented CLI argument parsing for player DLL names, number of rounds, display on / off / rate, text log on / off.
			- Changed game engine API accordingly. Refer to [2.b], 'Anatomy of the game engine'.

6. Future development (TODO list)
---------------------------------
- Implement graphic display rate control mechanism.
- Implement "simple" log mode.
