...
%%          'noiseNum' is the noise number to search. Any of the search
...
%%          If all of the the search parameters are '-1', the function will 
%%          return a table that contains all triggers which produced a -1 result
...
function dataTable = GetAVSData(resultsTable, trigNum, distNum, noiseNum)
  ...
  curDataRow = 1;
  %% Search for all '-1' results  
  if ((-1 == trigNum) && (-1 == distNum) && (-1 == noiseNum))
  ...
  %% Search all results based on search parameters
  for curRow = 1:numOfRows
    isCurDataRelevant = 0;
    curStr = char(resultsTable(curRow, 6));
    if (strcmp(curStr, "-1"))
